...
public class DynamicGraphApp extends ApplicationFrame implements GAConstants {
	...
	public DynamicGraphApp(final String title) {
		...
		dataset.addSeries(noData(), 0, "Tile");
		dataset.addSeries(noData(), 1, "Score");
		...
		timer = new Timer(FAST, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				generation++;
				float dataPoint = population.getFittest().getMaxTile();
				float scorePoint = population.getFittest().getScore();
	...
	private JFreeChart createChart(final XYDataset dataset) {
		final JFreeChart result = ChartFactory.createTimeSeriesChart(
				GRAPH_TITLE, "Generation", "Fitness/Score", dataset, true, true, false);
		final XYPlot plot = result.getXYPlot();
		ValueAxis domain = plot.getDomainAxis();
		domain.setAutoRange(true);
		ValueAxis range = plot.getRangeAxis();
		range.setRange(0, GRAPH_Y_MAX);
		return result;
	}
