...
public class GeneticAlgorithm implements GAConstants {
	...
	private Individual crossOver(Individual firstIndividual, Individual secondIndividual) {
		// gets smaller number of moves
		int numMoves = firstIndividual.getNumMoves() > secondIndividual.getNumMoves() ?
						secondIndividual.getNumMoves() : firstIndividual.getNumMoves();
						
		Individual newSolution = new Individual(numMoves);
	...
	private void mutate(Individual individual) {
		for (int i = 0; i < individual.getNumMoves(); i++) {
			if (Math.random() <= MUTATION_RATE) {
				Move gene = individual.generateMove();
